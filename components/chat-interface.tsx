...
import { ChatInterfaceProps } from '@/interface';
...
export default function ChatInterface({ documents, starterQuestions, handleSignIn }: ChatInterfaceProps) {
    ...
    async function chatDocument(question: string): Promise<void> {
        try {
            ...
            const response = await axios('https://trektripster-server-one.vercel.app/chat-document',
                // const response = await axios('http://127.0.0.1:8000/chat/chat-document',
                {
                    method: 'post',
                    headers: {
            ...
            setMessages((current) => [...current, { content: response?.data, role: 'assistant' }])
    ...
    return (
        <div className="relative rounded-[1.75rem] border border-border bg-card p-3 shadow-[0_24px_80px_-32px_rgba(16,36,48,0.35)] sm:p-4">
            <div className="flex h-[min(42rem,calc(100dvh-8rem))] min-h-0 overflow-hidden rounded-2xl border border-border bg-background">
